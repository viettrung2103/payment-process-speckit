...
public interface DeadLetterQueueRepository extends JpaRepository<DeadLetterQueueEntry, UUID> {
    ...
    List<DeadLetterQueueEntry> findByFailedAction(String failedAction);
    List<DeadLetterQueueEntry> findByPaymentIdAndFailedAction(UUID paymentId, String failedAction);
}